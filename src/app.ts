...
function putEmails() {
  ...
  (document.querySelector('.email-container') as HTMLDivElement).innerText =
    '(mailto:' + _email + ')';
  (document.querySelector('.call') as HTMLDivElement).setAttribute(
    'href',
    'tel:' + _phone
  );
  document
    .querySelector('.button.mail')
    .setAttribute('href', 'mailto:' + _email);
}
putEmails();
...
function setupListners() {
  ...
  /* fab-button */
  document
    .querySelector('button.fab-button')
    .addEventListener('click', function () {
  ...
  window.addEventListener('scroll', function () {
    (
      document.querySelector('.col-personal') as HTMLDivElement
    ).style.backgroundPositionY = window.pageYOffset / 2 + 'px';
    (
      document.querySelector('.mobile-nav') as HTMLDivElement
    ).style.opacity = `${
      (window.pageYOffset * 2 - screen.width) / screen.width
    }`;
  });
}
setupListners();
